...
+ (void)show:(NSString *)text toView:(UIView *)view
{
    ...
    NSDictionary *attributes = @{ NSFontAttributeName : [UIFont systemFontOfSize:14], NSParagraphStyleAttributeName : style };
    resultSize = [lbl.text boundingRectWithSize:size
                                   options: NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading
                                attributes:attributes context:nil].size;
    
    lbl.frame = CGRectMake(0, 0, resultSize.width, resultSize.height);
    viewContent.frame = CGRectMake(0, 0, resultSize.width > 40 ? resultSize.width : 40,  resultSize.height);
    HUD.customView = viewContent;
    HUD.mode = MBProgressHUDModeCustomView;
    //HUD.margin = 20;
