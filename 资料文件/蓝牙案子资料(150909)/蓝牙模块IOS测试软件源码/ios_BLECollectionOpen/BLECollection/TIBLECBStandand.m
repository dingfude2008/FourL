...
//    else {                      //列表中有曾经发现的设备，如果重复发现则刷新，
        for(int i = 0; i < self.activeDescriptors.count; i++) {
            ...
            if (p.characteristic.UUID == descriptor.characteristic.UUID) {
                NSLog(@"%d > %d !!!!!!!!! \r\n",i, (int)self.activeDescriptors.count);
...
//    }
    ...
    NSLog(@"activeDescriptors count = %d",(int)[self.activeDescriptors count]);
    NSLog(@"activeCharacteristics count = %d",(int)[self.activeCharacteristics count]);
}
...
-(void) DisplayCharacteristicDescriptorMessage:(CBDescriptor *)d{
    ...
    NSLog(@" value.length:%d",(int)c.value.length);
    ...
    NSLog(@"isBroadcasted:%@",@(c.isBroadcasted));
    NSLog(@"  isNotifying:%d\r\n",c.isNotifying);
...
-(NSString *)getUUIDString
{
    ...
    NSString *auuid = [[NSString alloc]initWithFormat:@"%@", activePeripheral.identifier.UUIDString];
    if (auuid.length >= 36) {
